...
function updateSubjectStats() {
    ...
    const anchorTime = getSubjectAnchorTime ? getSubjectAnchorTime(activeSubject) : state.customDate;
    const coords = getCelestialCoordinates ? getCelestialCoordinates(activeSubject.lat, activeSubject.lng, anchorTime) : null;
    ...
    // CZ (Celestial Zenith)
    const statsCZ = getImaginaryPointStats(cZenith.ra * 15, cZenith.dec, state.customDate, activeSubject, reqConfig);
    const czInfo = document.querySelector('#celestial-card-subject-celestial-zenith .celestial-info');
    if (czInfo && !czInfo.classList.contains('hidden') && statsCZ) {
        renderCardStats('stats-subject-celestial-zenith', statsCZ);
    }
    
    // CN (Celestial Nadir)
    const statsCN = getImaginaryPointStats(cNadir.ra * 15, cNadir.dec, state.customDate, activeSubject, reqConfig);
    const cnInfo = document.querySelector('#celestial-card-subject-celestial-nadir .celestial-info');
    ...
    const statsTZ = getTerrestrialStaticStats(true, state.customDate, activeSubject, reqConfig);
    const tzInfo = document.querySelector('#terrestrial-card-subject-terrestrial-zenith .terrestrial-info');
    ...
    const statsTN = getTerrestrialStaticStats(false, state.customDate, activeSubject, reqConfig);
    const tnInfo = document.querySelector('#terrestrial-card-subject-terrestrial-nadir .terrestrial-info');
