...
function getActiveMaps() {
    if (currentMapMode === 'CHART' || currentMapMode === 'CONFIG' || currentMapMode === 'MATRIX') {
        return [];
    }
    return map ? [map] : [];
}
...
function setMapMode(mode) {
    ...
    const chartMatrixContainer = document.getElementById('chart-matrix-container');
...
function initMap() {
    ...
    if (themeBtn) {
        ...
        if (savedTheme === 'dark' || (savedTheme === null && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
            ...
            if (currentTileLayer) {
                currentTileLayer.setUrl('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png');
            }
        } else {
            document.documentElement.classList.add('light');
            document.documentElement.classList.remove('dark');
            if (currentTileLayer) {
                currentTileLayer.setUrl('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png');
            }
        }
    ...
    if (splitContainer && dotWest && dotEast) {
        const updateDotActiveStates = () => {
            ...
            if (width > 0) {
                ...
                if (isEast) {
                    dotWest.classList.remove('bg-black', 'dark:bg-white', 'ring-2', 'ring-black', 'dark:ring-white', 'ring-offset-2', 'ring-offset-white', 'dark:ring-offset-black');
                    dotWest.classList.add('bg-black/40', 'dark:bg-white/40');
                    dotEast.classList.remove('bg-black/40', 'dark:bg-white/40');
                    dotEast.classList.add('bg-black', 'dark:bg-white', 'ring-2', 'ring-black', 'dark:ring-white', 'ring-offset-2', 'ring-offset-white', 'dark:ring-offset-black');
                } else {
                    dotEast.classList.remove('bg-black', 'dark:bg-white', 'ring-2', 'ring-black', 'dark:ring-white', 'ring-offset-2', 'ring-offset-white', 'dark:ring-offset-black');
                    dotEast.classList.add('bg-black/40', 'dark:bg-white/40');
                    dotWest.classList.remove('bg-black/40', 'dark:bg-white/40');
                    dotWest.classList.add('bg-black', 'dark:bg-white', 'ring-2', 'ring-black', 'dark:ring-white', 'ring-offset-2', 'ring-offset-white', 'dark:ring-offset-black');
                }
            }
        };
...
function renderChartSVG(type, activeSubject, date) {
    ...
    let siriusAz = 0;
    const siriusStats = getBodyStats('Sirius', date, activeSubject, { coord: state.coordConfig, zodiac: state.zodiacConfig });
    if (siriusStats) {
    ...
    const centerEntityId = isZenith ? 'subject-terrestrial-zenith' : 'subject-terrestrial-nadir';
    ...
    if (isZenith && state.showZenith && state.activeBodies.has('subject-celestial-zenith')) {
        const anchorTime = getSubjectAnchorTime(activeSubject);
        const coords = getCelestialCoordinates(activeSubject.lat, activeSubject.lng, anchorTime);
        if (coords) {
            const czAzAlt = getFixedCoordinateAzAlt(coords.zenith.ra, coords.zenith.dec, date, activeSubject);
            if (czAzAlt && czAzAlt.alt >= 0) {
                ...
                const czEntity = ENTITIES.find(e => e.id === 'subject-celestial-zenith');
                if (czEntity) {
    ...
    } else if (!isZenith && state.showNadir && state.activeBodies.has('subject-celestial-nadir')) {
        const anchorTime = getSubjectAnchorTime(activeSubject);
        const coords = getCelestialCoordinates(activeSubject.lat, activeSubject.lng, anchorTime);
        if (coords) {
            const cnAzAlt = getFixedCoordinateAzAlt(coords.nadir.ra, coords.nadir.dec, date, activeSubject);
    ...
    BODIES.forEach(b => {
        ...
        const stats = getBodyStats(b.id, date, activeSubject, { coord: state.coordConfig, zodiac: state.zodiacConfig });
        if (!stats) return;
...
function updateMap() {
    ...
    if (activeSubject) {
        // Zenith (TZ)
        if (!state.showZenith || !state.activeBodies.has('subject-terrestrial-zenith')) {
            removeMarker('zenith');
        } else {
            ...
            const tzEntity = ENTITIES.find(e => e.id === 'subject-terrestrial-zenith');
        ...
        // Celestial Zenith (CZ)
        if (!state.showZenith || !state.activeBodies.has('subject-celestial-zenith')) {
            removeMarker('cz');
        } else {
            const anchorTime = getSubjectAnchorTime(activeSubject);
            const coords = getCelestialCoordinates(activeSubject.lat, activeSubject.lng, anchorTime);
        ...
        // Nadir (TN)
        if (!state.showNadir || !state.activeBodies.has('subject-terrestrial-nadir')) {
            removeMarker('nadir');
        } else {
            ...
            const tnEntity = ENTITIES.find(e => e.id === 'subject-terrestrial-nadir');
        ...
        // Celestial Nadir (CN)
        if (!state.showNadir || !state.activeBodies.has('subject-celestial-nadir')) {
            removeMarker('cn');
        } else {
            const anchorTime = getSubjectAnchorTime(activeSubject);
            const coords = getCelestialCoordinates(activeSubject.lat, activeSubject.lng, anchorTime);
