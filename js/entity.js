...
const ENTITIES = [
    // 1. TERRESTRIAL ZENITH
    {
        ...
        getMapMarker: () => {
            ...
            iconElement.className = 'w-7 h-7 bg-white text-slate-950 rounded-full border-2 border-slate-950 shadow-md flex items-center justify-center text-xs font-mono font-bold transition-transform transform hover:scale-110 relative z-20';
    ...
    // 4. CELESTIAL NADIR
    {
        ...
        getMapMarker: () => {
            const iconElement = document.createElement('div');
            iconElement.className = 'w-7 h-7 bg-slate-950 dark:bg-white rounded-full border border-white dark:border-slate-950 shadow-md flex items-center justify-center text-xs font-mono font-bold text-white dark:text-slate-950 transition-transform transform hover:scale-110 relative z-30';
...
function generateStatsTableHTML(stats, isObject, bodyId = null) {
    ...
    if (isObject && bodyId) {
        const activeSubject = state.subjects.find(s => s.id === state.selectedSubjectId) || state.subjects[0];
        const zenithDate = typeof predictConjunction === 'function' ? predictConjunction(bodyId, state.customDate, activeSubject, true) : null;
        const nadirDate = typeof predictConjunction === 'function' ? predictConjunction(bodyId, state.customDate, activeSubject, false) : null;
...
function triggerSingleEntityUpdate(entityId) {
    ...
    if (entity.type === 'OBJECT') {
        stats = typeof getBodyStats === 'function' ? getBodyStats(entity.id, state.customDate, activeSubject, reqConfig) : null;
        isObject = true;
    } else if (entity.type === 'TERRESTRIAL') {
        stats = typeof getTerrestrialStaticStats === 'function' ? getTerrestrialStaticStats(entity.isZenith, state.customDate, activeSubject, reqConfig) : null;
    } else if (entity.type === 'CELESTIAL') {
        const anchorTime = typeof getSubjectAnchorTime === 'function' ? getSubjectAnchorTime(activeSubject) : state.customDate;
        const coords = typeof getCelestialCoordinates === 'function' ? getCelestialCoordinates(activeSubject.lat, activeSubject.lng, anchorTime) : null;
        if (coords) {
            const coordPoint = entity.isZenith ? coords.zenith : coords.nadir;
            stats = typeof getImaginaryPointStats === 'function' ? getImaginaryPointStats(coordPoint.ra * 15, coordPoint.dec, state.customDate, activeSubject, reqConfig) : null;
        }
    }
...
function getChartMarkerSVG(entity, px, py, sectorString = '') {
    ...
    // Default for celestial body OBJECT types
    const labelAbbrev = entity.id === 'Sirius' ? 'SUI' : entity.label.substring(0, 3).toUpperCase();
